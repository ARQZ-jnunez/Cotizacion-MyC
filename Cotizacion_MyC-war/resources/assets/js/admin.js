/*
 * Admin of Cotizacion_MyC
 */

/* global global */

let $ = global.jQuery = global.$ = require('jquery');
...
let sha512 = global.sha512 = require('sha512');
let toastr = global.toastr = require('toastr');
...
$(document).ready(function () {
    ...
    $("input#inputRut").rut({
        ...
        toastr.error("El rut " + this.value + " no es válido.");
...
let validateEmail = global.validateEmail = function validateEmail(emailField) {
    let reg = /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;

    if (!reg.test(emailField.value)) {
        toastr.error("Correo no es válido");
        return false;
    }
...
let valid_rut = global.valid_rut = function isValidRUT(rut) {
...
let delete_confirm = global.delete_confirm = function delete_confirm(route) {
    $.confirm({
        title: '¿Eliminar Registro?',
        content: '¿Estás seguro que deseas eliminar este registro?. El mensaje se cerrara en 8 segundos.',
